...
int8_t OV5640::singleFocus() {
  if (!isOV5640) return -1;

  int8_t rc = 0;
  rc = sensor->set_reg(sensor, OV5640_CMD_ACK, 0xff, 0x01);
  rc = sensor->set_reg(sensor, OV5640_CMD_MAIN, 0xff, AF_TRIG_SINGLE_AUTO_FOCUS);
  if(rc < 0)
  {
    return -1;
  }
